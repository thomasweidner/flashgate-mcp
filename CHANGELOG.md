...
- Closed the MCP revision-planning documentation gaps without changing runtime behavior: `BACKLOG.md` now carries the authoritative BL-207 → BL-208 → BL-204/212/219 → BL-209/210/211 implementation order and the explicit no-advertising gate for `2026-07-28`; active protocol documentation now names revision-specific `2025-11-25` and `2026-07-28` behavior instead of generic `legacy`/`modern` MCP generation labels, while deprecated MCP Roots compatibility is scoped to demonstrated supported `2025-11-25` client need.
- Converged the agent-efficiency/content-identity plan without changing runtime behavior or adding backlog IDs. BL-048 now owns reusable content identities as non-authorizing fingerprint evidence; Version 1.0 BL-346 owns compact batch expected-state verification and explicit fresh/strong proof, while post-Version-1.0 BL-217 owns conditional/not-modified retrieval and optional execution-context-bound server caches. BL-208 now records the final `2026-07-28` `resources/read` cache-hint and `subscriptions/listen` change-notification semantics. BL-350 is explicitly one portable FlashGate skill with progressive references and no Voxtronic/INF, consumer `AGENTS.md`, Codex-Work, task/handoff, or agent-product dependency. Active architecture/scope/roadmap documentation also corrects BL-346 to Version 1.0 and keeps BL-217/BL-350 post-Version-1.0.
