...
func TestAllToolsRejectMalformedUnknownTrailingAndWrongTypes(t *testing.T) {
	tests := []struct {
		...
		{"edit_file", NewEditFileTool(newFakeFileSystem())},
...
func TestRequiredPathsRejectMissingEmptyAndWhitespace(t *testing.T) {
	tests := []struct {
		...
		{"edit_file", NewEditFileTool(newFakeFileSystem()), `{}`, `{"path":"","startByte":0,"endByte":0,"content":""}`, `{"path":"  ","startByte":0,"endByte":0,"content":""}`},
