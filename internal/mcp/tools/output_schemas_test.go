...
func TestFilesystemRuntimeOutputSchemas(t *testing.T) {
	...
	runtimeTools := []Tool{
		...
		NewWriteFileTool(fake), NewAppendFileTool(fake), NewCreateDirectoryTool(fake), NewDeletePathTool(fake),
...
func TestFilesystemStructuredResultsMatchOutputSchemas(t *testing.T) {
	tests := []struct {
		...
		{appendFileToolName, appendFileResult{Path: "file.txt", Size: 4, Appended: true}},
...
func TestKnownOutputSchemaPropertyTypes(t *testing.T) {
	tests := []struct {
		...
		{appendFileToolName, "appended", "boolean"},
