...
func (f *fakeFileSystem) WriteWithMode(path string, content []byte, mode fs.WriteMode) error {
	f.writePath, f.writeContent = path, append([]byte(nil), content...)
	f.writeOverwrite = mode != fs.WriteCreateOnly
	f.writeMode = mode
	return f.writeErr
}
