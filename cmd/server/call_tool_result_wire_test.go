...
func TestFilesystemCallToolWireErrorsRemainSafe(t *testing.T) {
	...
	tests := []struct {
		...
		{"invalid arguments", defaultRegistry, `{"name":"get_path_info","arguments":{}}`, "invalid params", "invalid_arguments"},
		{"PathGuard traversal", defaultRegistry, `{"name":"read_file","arguments":{"path":"..\\outside.txt"}}`, "filesystem error: invalid path", "invalid_path"},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.category == "" {
				if response.Error == nil || response.Error.Code != protocol.ErrInvalidParams || response.Error.Message != tc.message || len(response.Result) != 0 {
					t.Fatalf("unexpected protocol error response: %s", raw)
				}
			} else {
				if response.Error != nil {
					t.Fatalf("tool failure used JSON-RPC error channel: %s", raw)
				}
				var result protocol.CallToolResult
				if err := json.Unmarshal(response.Result, &result); err != nil {
					t.Fatalf("decode tool error result: %v; response=%s", err, raw)
				}
				var body protocol.ToolError
				if err := json.Unmarshal(result.StructuredContent, &body); err != nil || !result.IsError || body.Category != tc.category || body.Message != tc.message {
					t.Fatalf("unexpected normalized tool error: %#v, decode=%v; response=%s", result, err, raw)
				}
			}
