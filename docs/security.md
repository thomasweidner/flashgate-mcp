...
- targeted byte-range and expected-match-count edits confined through `PathGuard`, with source and result sizes bounded by `MCP_MAX_WRITE_BYTES` and stale or ambiguous match edits rejected before writing
