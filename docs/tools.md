...
The read-only profile exposes `list_directory`, `read_file`, `read_files`, and `get_path_info`. Write-capable tools are not registered in read-only mode, and calls to unavailable or unknown names return generic JSON-RPC Invalid params.
...
The current deterministic UTF-8 JSONL response snapshot, including its trailing newline, is 3340 bytes for read-only and 6863 bytes for default. Matching hard budgets detect unreviewed tool/catalog drift.
...
Required: `paths`, an ordered array of 1 to 100 relative paths. Optional: `maxBytesPerFile`, with the same server cap as `read_file`. The sum of successful content bytes is also capped at the server file-size limit, preventing a batch from multiplying the memory and response budget.
...
Results preserve input order. A failure for one item does not discard successful reads; errors use bounded categories and never include raw operating-system details. `accepted`, `completed`, `failed`, and `totalBytes` summarize the batch.

```json
{
  "results": [
    {"path": "README.md", "content": "...", "size": 123},
