...
The current deterministic UTF-8 JSONL `tools/list` response, including its trailing newline, is 6148 bytes for read-only and 9671 bytes for default. The older 1239/2134-byte, 3850/5657-byte, 3046/6569-byte, 4271/7794-byte, 4894/8417-byte, 5594/9117-byte, and 5951/9474-byte measurements remain historical snapshots, not persistent payload budgets.
...
Optional `pageSize` from 1 through 100 enables pagination for path and content results without changing the deterministic logical order or the existing search budgets. When more results remain, the response includes an opaque `nextCursor`. Continue with a request containing only that value as `cursor`; query fields and `pageSize` cannot accompany a cursor. Cursors are cryptographically random, server-held, single-use, scoped to the search tool instance (and therefore its configured root/profile), and expire after five minutes. Each successful continuation consumes the cursor and returns a fresh cursor when another page remains. Unknown, expired, replayed, or cross-instance cursors fail with the safe stale-cursor category. Omitting `pageSize` preserves the bounded unpaged response.

Optional `ignoreFile` explicitly selects one root-relative ignore file; no `.gitignore` or other conventional file is read by default. Its gitignore-compatible rules support comments, negation, root anchoring, directory-only rules, `*`, `**`, `?`, and character classes. Rules are case-sensitive, relative to the ignore file's directory, and applied in file order with the last match winning; ignored directories are pruned. The file is read through the same root/path policy and is bounded to 64 KiB, 256 active patterns, and 1,024 bytes per line. Invalid paths, UTF-8, syntax, or bounds fail closed before search traversal.
