...
Files and directories may be renamed or moved on the same volume. Cross-volume moves are currently rejected without copy/delete fallback. The Version 1.0 target copy/verify/delete behavior, cancellation points, and partial-completion rules are defined in the [cross-volume move contract](cross-volume-move-contract.md); that workflow is not implemented yet. With `overwrite:true`, only file-to-existing-file replacement is allowed. Existing directory targets and all directory replacement combinations are rejected. Same-path, same-file (including detectable hardlinks and Windows case aliases), and lexical or symlink-resolved directory-into-own-subtree operations are rejected before replacement.
