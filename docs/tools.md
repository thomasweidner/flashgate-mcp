...
Every search uses fixed server-owned traversal caps of 64 descendant levels and 10,000 visited entries. Entry accounting occurs before result filtering or content reads, so a selective query cannot turn a wide tree into an unbounded scan. Content scanning additionally permits at most 1,000 opened files, 1 MiB per file, 10 MiB scanned across the request, 256 matches per file, 1,000 matches total, and 1 MiB for the encoded match collection. The operation fails closed with a safe limit error before crossing any cap, checks cancellation during traversal and between returned matches, and returns no partial result on failure. The same caps apply to literal and regular-expression search. Client-selected limits are not exposed; match-limit refinements remain owned by BL-076, context by BL-077, and pagination by BL-079.
