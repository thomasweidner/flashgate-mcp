...
Focused contract tests compare runtime tool definitions with `docs/mcp-tool-catalog.json` for name, title, description, complete input schema, and deeply equal runtime `outputSchema`/catalog `resultSchema`. The profile-budget contract test loads `docs/mcp-profile-budgets.json` strictly and checks each live profile's tool count, complete `tools/list` response bytes, compact input/output schema bytes, per-tool and aggregate description bytes, and initialization-instruction bytes. Targeted tests require exactly eight runtime output schemas, object roots, valid required/property relationships, expected project property types, representative successful `structuredContent`, both `get_path_info` variants, and the `read_file` outer-array/inner-string distinction. The tests-only structural checker covers only `type`, `properties`, `required`, `additionalProperties`, `items`, `oneOf`, and `const` as currently emitted; it is not a complete JSON Schema 2020-12 validator.

The `tools/list` JSON-RPC wire test checks schema exposure for both profiles and records deterministic UTF-8 JSONL sizes with and without output schemas. `SPR-046` records 1239/2134 bytes for read-only and 3850/5657 bytes for default. The broader Version 1.0 ceilings are enforced separately by the [profile budget contract](mcp-profile-budget-contract.md); these historical exact snapshots remain useful change diagnostics.
