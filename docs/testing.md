...
The `tools/list` JSON-RPC wire test checks schema exposure for both profiles and records deterministic UTF-8 JSONL sizes with and without output schemas. `SPR-046` records 1239/2134 bytes for read-only and 3850/5657 bytes for default; no regression budget is enforced. Initialize-handler tests require the exact profile-specific server instructions and keep each instruction below 512 UTF-8 bytes without claiming the profile-wide budgets owned by `BL-215` and `BL-256`.
