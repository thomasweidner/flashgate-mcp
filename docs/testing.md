...
The `tools/list` JSON-RPC wire test checks schema exposure for both profiles and records deterministic UTF-8 JSONL sizes with and without output schemas. The current deterministic snapshots are 3340 bytes for read-only and 6863 bytes for default; the matching hard budgets detect unreviewed catalog drift.
...
### Batch file-read contract

Focused `read_files` tests enforce whole-batch preflight, the 100-path cap, deterministic input-order results, partial failures without raw host errors, per-file limits, and the aggregate successful-byte cap. Runtime/catalog schema parity and both profile registries permanently cover the batch tool.
