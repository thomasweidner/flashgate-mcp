...
The `tools/list` JSON-RPC wire test checks schema exposure for both profiles and records deterministic UTF-8 JSONL sizes with and without output schemas. `SPR-046` recorded 1239/2134 bytes for read-only and 3850/5657 bytes for default. Paginated `list_directory` and bounded `get_directory_tree` change the current with-schema snapshots to 4126 bytes for read-only and 7649 bytes for default; no regression budget is enforced by this wire test.
