...
The Windows/Linux CI test matrix also runs the focused
`TestCallToolResultSerialization*` payload gate explicitly before the coverage
suite. This makes exact response-size regression checks, the versioned payload
ceilings, complete fixture-to-budget coverage, and allocation ceilings a named
required CI boundary rather than relying only on their incidental inclusion in
the repository-wide test run. Race-instrumented executions retain the
functional payload checks and skip only allocation counting because race
