...
The `tools/list` JSON-RPC wire test checks schema exposure for both profiles and records deterministic UTF-8 JSONL sizes with and without output schemas. `SPR-046` records the historical 1239/2134-byte read-only and 3850/5657-byte default baselines; BL-048 adds `hash_files`, producing 3362 bytes for read-only and 6885 bytes for default; no regression budget is enforced.
