...
The planned Search implementation must satisfy the permanent matrix in the [search model and threat model](search-model-and-threat-model.md). Focused gates cover deterministic ordering, root/link/reparse confinement, every incremental resource limit, client match-limit validation, bounded total/per-file truncation diagnostics, cancellation and deadlines, pattern/regex rejection, binary and encoding outcomes, file changes during scans, cursor binding and invalidation, capability bypass attempts, and safe diagnostics. Native Windows and Linux validation is required for platform path and filesystem behavior; Cloud-only tests cannot establish that evidence.

Focused content-search tests cover literal and Go RE2-style regular-expression matching with deterministic non-overlapping UTF-8 byte offsets, composition with filename and file metadata filters, pre-traversal rejection of invalid selectors, regex syntax, and match limits, root-confined bounded reads, cancellation, and incremental file/scan-byte/match/response ceilings. They also cover NUL-based binary detection, malformed UTF-8 classification, bounded skip diagnostics, fail-closed error mode, and explicit byte-search opt-in without context. Alternate encodings are rejected rather than guessed or transcoded. Pagination tests cover deterministic path and content page boundaries, preserved match-limit diagnostics, single-use cursor rotation, replay and expiry rejection, cursor/query isolation, and page-size validation.

Ignore-file tests prove that no file is read without explicit opt-in, the selected path stays behind the root-confined reader, gitignore-compatible comments/globs/anchoring/directory rules and last-match negation are deterministic, ignored directories are pruned, and malformed UTF-8, syntax, oversized files, excess patterns, and overlong lines fail before search traversal. Native Windows finalization must confirm separator, case, and reparse behavior with real filesystems.
...
The `tools/list` JSON-RPC wire test checks schema exposure for both profiles and records deterministic UTF-8 JSONL sizes with and without output schemas. The current nine-tool catalog records 6148 bytes for read-only and 9671 bytes for default; the earlier 1239/2134-byte, 3850/5657-byte, 3046/6569-byte, 4271/7794-byte, 4894/8417-byte, 5594/9117-byte, and 5951/9474-byte measurements remain historical, and no regression budget is enforced.
...
The implemented protocol remains MCP `2025-11-25`. Explicit `CallToolResult` DTO tests, a strict project-local decoder, legacy unwrapped negative fixtures, all-nine-tool adapter coverage, and full JSON-RPC wire tests cover success and the unchanged error contract. The decoder intentionally validates the exact FlashGate-emitted subset (one text block, required object `structuredContent`, optional boolean `isError`, no `_meta`) rather than claiming to decode every standard-conformant MCP result. Windows and Bash positive smokes enforce the same shape.
