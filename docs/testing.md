...
The `tools/list` JSON-RPC wire test checks schema exposure for both profiles and records deterministic UTF-8 JSONL sizes with and without output schemas. The current nine-tool catalog records 4894 bytes for read-only and 8417 bytes for default; the earlier 1239/2134-byte, 3850/5657-byte, 3046/6569-byte, and 4271/7794-byte measurements remain historical, and no regression budget is enforced.
