...
Focused search tests cover literal and Go RE2-style regular-expression matching with deterministic non-overlapping UTF-8 byte offsets, composition with filename, file metadata, and normalized root-relative include/exclude filters, and exclusion precedence. They also verify bounded pattern count/length, pre-traversal rejection of empty, duplicate, malformed, or backslash-containing patterns, traversal through nonmatching directories, and that excluded content files are not opened. Existing gates retain root-confined bounded reads, cancellation, and incremental file/scan-byte/match/response ceilings. Alternate encodings, binary classification, client-selected limits, context, and pagination remain assigned to their separate Search tasks.
...
The `tools/list` JSON-RPC wire test checks schema exposure for both profiles and records deterministic UTF-8 JSONL sizes with and without output schemas. The current nine-tool catalog records 4271 bytes for read-only and 7794 bytes for default; the earlier 1239/2134-byte, 3850/5657-byte, and 3046/6569-byte measurements remain historical, and no regression budget is enforced.
