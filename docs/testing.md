...
`get_paths_info` tests cover ordered existing/missing/failure results, counters, strict arguments, the 100-item preflight cap, no filesystem access for invalid batches, output-schema parity, and the complete `tools/call` wire envelope.
...
The `tools/list` JSON-RPC wire test checks schema exposure for both profiles and records deterministic UTF-8 JSONL sizes with and without output schemas. `SPR-046` records 1239/2134 bytes for read-only and 3850/5657 bytes for default; no regression budget is enforced. The BL-047 catalog expansion records current response sizes of 3527 bytes for read-only and 7050 bytes for default.
