...
The `benchmark-baseline-comparison` CI job runs
`TestCompleteVersionedPlatformBaselines` as a dedicated permanent gate. It fails
when either required versioned baseline is absent or invalid, when a deterministic
hard budget is exceeded, or when the Windows and Linux deterministic projections
diverge. This focused comparison does not execute or record host-sensitive
benchmarks and does not reinterpret soft review budgets as hard failures.
