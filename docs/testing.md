...
Focused contract tests compare runtime tool definitions with `docs/mcp-tool-catalog.json` for name, title, description, complete input schema, and deeply equal runtime `outputSchema`/catalog `resultSchema`. Every schema root must declare `https://json-schema.org/draft/2020-12/schema`; the permanent schema-contract gate recursively validates every emitted keyword and keyword value, required/property consistency, non-empty `oneOf`, numeric constraints, and deterministic serialization. Targeted instance tests require exactly nine runtime output schemas, object roots, expected project property types, representative successful `structuredContent`, both `get_path_info` variants, the `system_info` string fields, and the `read_file` outer-array/inner-string distinction.
...
Future protocol or extension support still requires version-negotiation, extension-negotiation, client fallback, and compatibility tests before it is advertised. Official MCP conformance tooling remains planned; current input and output schemas are validated as JSON Schema 2020-12 without changing the advertised MCP `2025-11-25` revision.
