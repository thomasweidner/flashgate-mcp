...
The `tools/list` JSON-RPC wire test checks schema exposure for both profiles and records deterministic UTF-8 JSONL sizes with and without output schemas. The current complete responses are 2134 bytes for read-only and 6069 bytes for default, including BL-053 conditional-write inputs; benchmark budgets enforce those catalog payloads.
