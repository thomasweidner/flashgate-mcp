...
The `tools/list` JSON-RPC wire test checks schema exposure for both profiles and records deterministic UTF-8 JSONL sizes with and without output schemas. After `BL-055`, it records 1239/2134 bytes for read-only and 4323/6326 bytes for default; no regression budget is enforced.
