...
Focused contract tests compare runtime tool definitions with `docs/mcp-tool-catalog.json` for name, title, description, complete input schema, and deeply equal runtime `outputSchema`/catalog `resultSchema`. The committed [`internal/mcp/tools/testdata/tool-schemas.json`](../internal/mcp/tools/testdata/tool-schemas.json) snapshot additionally binds every complete runtime input/output schema, runtime tool order, and the advertised protocol revision through an exact deterministic-byte comparison. Snapshot changes require deliberate public-contract review; the [MCP conformance and schema snapshot assessment](mcp-conformance.md) defines the artifact and official-tooling boundary. Targeted tests require exactly eight runtime output schemas, object roots, valid required/property relationships, expected project property types, representative successful `structuredContent`, both `get_path_info` variants, and the `read_file` outer-array/inner-string distinction. The tests-only structural checker covers only `type`, `properties`, `required`, `additionalProperties`, `items`, `oneOf`, and `const` as currently emitted; it is not a complete JSON Schema 2020-12 validator.
...
Future protocol or extension support still requires version-negotiation, extension-negotiation, client fallback, and compatibility tests before it is advertised. Complete JSON Schema 2020-12 validation remains planned under BL-212. The official MCP conformance framework has been evaluated under BL-204 but is not a permanent gate while its server runner requires HTTP and FlashGate ships only STDIO; see [MCP conformance and schema snapshots](mcp-conformance.md).
