...
Focused contract tests compare runtime tool definitions with `docs/mcp-tool-catalog.json` for name, title, description, complete input schema, and deeply equal runtime `outputSchema`/catalog `resultSchema`. Focused `list_directory` tests also require omitted `fields` to preserve the complete entry shape and validate exact projection plus rejection of empty, duplicate, and unknown selections before filesystem access. Targeted tests require exactly eight runtime output schemas, object roots, valid required/property relationships, expected project property types, representative successful `structuredContent`, both `get_path_info` variants, and the `read_file` outer-array/inner-string distinction. The tests-only structural checker covers only `type`, `properties`, `required`, `additionalProperties`, `items`, `oneOf`, and `const` as currently emitted; it is not a complete JSON Schema 2020-12 validator.

The `tools/list` JSON-RPC wire test checks schema exposure for both profiles and records deterministic UTF-8 JSONL sizes. The current field-selection-aware snapshots are 2329 bytes for read-only and 5852 bytes for default, with matching hard catalog budgets. The earlier `SPR-046` 1239/2134-byte read-only and 3850/5657-byte default measurements remain historical.
