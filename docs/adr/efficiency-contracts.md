...
Before 1.0, justified tool names, parameters, and results may change or be removed with coordinated tests, documentation, and migration guidance. The early filesystem name cleanup is implemented; see [migration](../migration.md#tool-names). Further contract enhancements remain owned by the backlog.

`get_path_info` represents genuine absence structurally with `exists: false`; access, root, path-type, and I/O failures remain distinct errors.
...
Record baselines before optimization. Use machine-readable errors and schema snapshots, and test all changes through MCP `tools/call`.
...
The implemented pre-1.0 baseline has exactly eight default tools in this order: `list_directory`, `read_file`, `get_path_info`, `write_file`, `create_directory`, `delete_path`, `copy_path`, and `move_path`. The read-only profile exposes the first three read tools. The five replaced or redundant names have no aliases.

Inputs are closed objects and are strictly decoded at runtime. `get_path_info` uses one metadata operation and represents genuine absence structurally; policy denials remain errors. `create_directory` reports actual creation state. `copy_path` remains file-only. `move_path` is the single same-volume Move/Rename contract with pre-replacement SameFile, effective-subtree, identity-revalidation, and overwrite safeguards. Directory copy and the full normalized tool-error migration remain deferred. Successful result wrapping and runtime output schemas are implemented as described below.

## Current successful result contract

The implemented adapter uses MCP `CallToolResult` variant B for every successful call from all eight filesystem tools: one `TextContent` block contains compact deterministic JSON, and `structuredContent` contains the same domain object. Protocol conformance takes precedence over minimizing the duplicated payload. Reproducible benchmarks record response bytes, runtime, and allocations for the historical direct form, text-only form, and selected text-plus-structured form without setting CI budgets from a single machine.

Typed domain results and the filesystem core remain independent of MCP. Runtime `outputSchema` is implemented for all eight tools and describes the typed domain result. The existing safe JSON-RPC tool-error model is not partially migrated; BL-203 remains responsible for a complete normalized `isError=true` contract.

## Measurement contract

The benchmark components introduced under BL-189 through BL-199 provide a three-layer measurement baseline. Current completion status is recorded in the backlog. Existing serialization fixtures remain authoritative and now also report complete JSON-RPC response bytes. Additional in-process benchmarks cover direct `tools/call` handling and schema-bearing `tools/list` responses for read-only and default profiles.
...
See [Efficiency Improvement Plan](../planning/efficiency.md) and `BL-213` through `BL-220`.
