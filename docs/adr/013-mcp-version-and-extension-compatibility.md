...
The `2026-07-28` adapter path owns per-request protocol/capability metadata, `server/discover`, unsupported-version errors, required result typing, server identity metadata, list cache hints, and extension negotiation. Self-reported client information, discovery results, extension capabilities, and cache state are explicitly non-authoritative for FlashGate security decisions. Existing prepared Mobile PRs for BL-207/208/209 must be freshly rebased and reviewed against this final-spec planning before integration.
