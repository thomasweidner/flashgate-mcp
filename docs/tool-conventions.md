...
The default profile exposes the path-search tool and all eight filesystem baseline tools. The read-only profile exposes exactly:
...
Client activation must set `MCP_READ_ONLY=true` explicitly; the missing-variable default contains the read-only search/filesystem tools plus the five write tools. The read-only and negative STDIO smokes require identical generic Invalid params responses for every write-gated and removed legacy name.
