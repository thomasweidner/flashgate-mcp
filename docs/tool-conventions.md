...
The default profile exposes nine tools, including the write-gated `edit_file` targeted-edit primitive. The read-only profile exposes exactly:
...
`write_file`, `edit_file`, `create_directory`, `delete_path`, `copy_path`, and `move_path` are write-gated and absent from the read-only registry.
