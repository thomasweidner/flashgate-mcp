...
| BL-260 | Planned | Keep standard test/vet/lint/build gates and raise product-code coverage | Preserve formatting, vet, tests, lint, build validation, and separate Windows/Linux coverage artifacts; migrate coverage to the FlashGate production package closure linked into `./cmd/server`, raise real product statement coverage and the hard per-platform merge gate together to at least 90.0%, target at least 95.0% and 100% where practical for bounded core logic, retain semantic/security/race tests beyond coverage, and keep the current `.github/workflows/ci.yml` thresholds authoritative until the migration passes |
