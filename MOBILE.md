...
This file intentionally lists only **new Mobile work that remains actionable without a new Classic/owner decision**. A BL task is excluded from the active catalog when any of these is true:
...
| `C — CLOUD_ANALYSIS_OR_CONTRACT` | Work crosses an architecture/product/policy/owner decision boundary | Excluded from this active list until Classic/owner decision is recorded |
...
6. select from the earliest sprint with an executable unreserved A/B candidate;
7. inside that sprint prefer mode A before B, independent before stack-base-ready, then lower effort, lower Windows residual and lower collision risk;
...
| `BL-223` | CLI mode/lifecycle ownership contract requires architecture decision closure before implementation |
| `BL-225` | Versioned local IPC protocol/compatibility contract requires architecture decision closure |
| `BL-233` | Configuration precedence, endpoint discovery and logging contract requires owner/architecture closure |
| `BL-235` | Hybrid per-root execution-identity choice is an architecture/security decision |
| `BL-238` | Variant-B user-worker contract/threat model remains architecture/security decision work |
...
These IDs were unreserved at the snapshot, are `Planned`, are Mode A/B, and are not blocked by a known Classic/owner decision. Reclassify real dependencies before every mutation.
