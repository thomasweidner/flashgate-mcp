...
7. If that sprint has no current-checkout executable candidate, perform the read-only **open-PR-head restart pass** from `Governance/MOBILE-CLOUD-HANDOFF.md` before advancing to the next sprint. For each remaining candidate, determine its complete hard-predecessor delta set and whether exactly one open PR head ancestry already contains all of it. One cumulative PR ancestry may satisfy several logical prerequisite BLs.
8. If one or more candidates in that sprint each have exactly one sufficient open PR head, choose among those candidates using the same mode/effort/Windows-residual/collision preferences and return `STACK_RESTART_REQUIRED` with the selected predecessor PR, head branch, head SHA, child BL, and exact restart instruction. Do **not** require the sprint to contain only one stack candidate; the single-predecessor condition is evaluated per candidate.
9. If a candidate genuinely requires two or more independent uncombined PR heads, classify it `BLOCKED_MULTIPLE_UNCOMBINED_MOBILE_PREDECESSORS`, exclude only that candidate, and continue. If the sprint has neither a current-checkout executable candidate nor a single-head restart candidate, continue with the next Planned sprint.
10. Do not attempt `git fetch`, manual remote configuration, merge, cherry-pick, patch replay, or synthetic predecessor import to make a `STACK_REQUIRED` candidate executable.
11. Return a global no-executable Planned result only after every allowed Planned sprint has completed both the current-checkout pass and, where needed, the open-PR-head restart pass.
12. Do not auto-select `Later` work while any executable or single-head-restartable unreserved `Planned` Mobile task exists.
13. A user-named task or epic explicitly overrides this automatic sprint preference, but a user-named task that is already reserved must be reported rather than duplicated unless the user explicitly authorizes a competing candidate.
