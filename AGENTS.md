...
`BLOCKED_MULTIPLE_UNCOMBINED_MOBILE_PREDECESSORS` is candidate-local. Exclude that candidate and continue evaluating other candidates and later Planned sprints. A reserved predecessor PR may still be a valid read-only-discovered start ref for an unreserved child; reservation prevents duplicate implementation of the predecessor, not legitimate stacking on its head. Multiple sufficient heads in one ancestry lineage are not an ambiguity after reduction: keep the earliest/minimal sufficient head whose ancestry already contains the complete hard-predecessor set.
