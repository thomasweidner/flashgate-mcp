...
1. Read the **complete, selector-stable, paginated** open-PR Vacation Reservation Ledger from GitHub exactly as defined in `Governance/MOBILE-CLOUD-HANDOFF.md`. Two immediately consecutive full pagination passes must produce the same selector-relevant PR projection before any reservation or restart analysis begins.
