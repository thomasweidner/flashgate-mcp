...
On Windows, the runner uses `OpenProcess`, `GetProcessMemoryInfo`, and `GetProcessTimes` to read current/peak working set plus user/kernel CPU time. On Linux, it reads `VmRSS` and `VmHWM` from `/proc/<pid>/status`, user/system CPU ticks from `/proc/<pid>/stat`, and the kernel-provided `AT_CLKTCK` value from `/proc/self/auxv` for conversion to nanoseconds. It does not assume a fixed 100 Hz clock. If the auxiliary-vector lookup fails or returns an invalid value, CPU metrics are reported as unsupported instead of using a guessed rate; independently available memory values remain present. Each missing value is named in `unsupported_metrics`. Other operating systems report resource status `not_supported`, omit unsupported numeric metrics, and never emit plausible zero placeholders.
