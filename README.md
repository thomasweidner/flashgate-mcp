...
The [benchmark method and baseline guide](docs/benchmark-method-and-baselines.md) explains how to interpret diagnostic runs and committed evidence. Detailed counter semantics, platform behavior, reference workflows, result schema, baseline, and hard-versus-soft budgets are documented in [`benchmarks/README.md`](benchmarks/README.md). The approximation `approx_tokens_bytes4 = ceil(UTF-8 bytes / 4)` is not model-specific and is not suitable for billing.
