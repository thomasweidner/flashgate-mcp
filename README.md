...
Runtime `outputSchema` is exposed for all nine tools and remains deeply equal to the catalog `resultSchema` values. `get_disk_usage` adds root-scoped capacity totals for an existing relative path without returning host paths or volume identifiers. These schemas describe successful `structuredContent`; the current safe JSON-RPC tool-error contract remains unchanged pending BL-203.
