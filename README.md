...
The current implemented scope is filesystem operations plus bounded, optionally paginated path, filename, metadata, literal-text, and regular-expression search slices with explicit optional ignore-file filtering. Version 1.0 plans further search capabilities, process observation/management, typed allowlisted command execution, controlled system information, named roots, safe-default capability profiles, the Operations/Job Manager, payload-efficient large-result handling, and optional local system-service deployment. These remain planned work.
...
| Tool | Description |
|---|---|
| `search_paths` | Recursively returns bounded root-relative paths or literal/RE2-style content matches in deterministic order. |
